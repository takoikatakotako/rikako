...
- **SSM Parameter Store でのシークレット管理**:
  ...
  - `/rikako/production/android/*`（upload-keystore / -password / key-alias / key-password / play-service-account）は Android の Play 用署名素材の**マスター**。`scripts/android-signing.sh push` が SSM と GitHub Secrets（CI が読む）の両方に書く。手元では `pull`（#405）
...
> - `database-url` は環境ごとに 1 本（dev は `/rikako/development/...`、prod は `/rikako/production/...`）。Terraform が Neon の `connection_uri` から**初期値だけ**登録し、`lifecycle.ignore_changes = [value]` を付けている。以降のローテーションは Terraform の管轄外（out-of-band）なので `aws ssm put-parameter --overwrite` で更新してよく、次の `terraform apply` で巻き戻ることはない。
