...
- **SSM Parameter Store でのシークレット管理**:
  ...
  - `/rikako/<env>/firebase/ios/<app_slug>`・`/rikako/<env>/firebase/android` は Firebase のクライアント設定（plist / google-services.json、git 管理外）。`scripts/firebase-config.sh pull|push` で出し入れ、手動登録
