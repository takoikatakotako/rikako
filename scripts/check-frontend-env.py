...
#   dev  : main への push（web/** と自ファイル）＋ 手動 ＋ Sync Content Dev からの workflow_call
#   prod : 手動 ＋ Deploy All Prod / Sync Content Prod からの workflow_call
#
...
# dev の workflow_call は data/** の反映（publish の後に web をビルドする順序の強制、#391）。
EXPECTED_TRIGGERS = {
    "dev": {"push", "workflow_dispatch", "workflow_call"},
